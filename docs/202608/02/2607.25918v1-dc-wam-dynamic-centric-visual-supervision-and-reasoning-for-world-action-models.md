...
tldr: 世界动作模型利用未来视觉预测提升机器人策略，但现有RGB重建过度关注纹理光照等弱相关外观信息。DC-WAM提出动态中心框架：监督端用时间差分流匹配与轨迹引导权重强化交互区域的视觉动态；推理端DynaRoute将token级动态相关性转为注意力偏置。实验表明在光照、物体外观和背景扰动下显著提升策略性能且无需额外在线输入。
source: arxiv
...
motivation: 现有WAM的未来视频预测被外观主导，纹理光照等与动作选择弱相关的信息消耗监督与计算，需转向控制相关的视觉动态。
method: 提出DC-WAM，监督层用时间差分流匹配和轨迹引导权重，推理层用DynaRoute生成注意力偏置，但不增加在线输入。
result: 在仿真与真实操作实验中，DC-WAM在光照、物体外观、背景纹理等分布外扰动下持续提升策略性能。
conclusion: 将WAM的视频分支从外观重建转向动态中心监督与推理，可有效增强控制相关的视觉表征，提升鲁棒性。
---

## 摘要
世界-动作模型（WAM）通过未来视觉预测增强机器人策略，但视觉模态应当为控制学习什么仍不明确。尽管逼真的未来预测提供了密集监督，但也带来了大量计算，并可能将能力分配给与动作选择弱相关的纹理、光照和背景变化。近期高效的WAM变体表明，视频分支的主要益处可能不在于渲染的未来本身，而在于训练过程中诱导出的与控制相关的视觉表征。在本工作中，我们从动态中心的视角重新审视未来视频预测，并探究是否可以将现有的基于RGB的WAM从外观主导的重构转向交互诱导的视觉动态，而无需在部署时引入额外的模态特定预测或在线输入。我们提出DC-WAM，一种动态中心的WAM框架，在RGB视频分支中重新分配监督与计算。在监督层面，DC-WAM结合时间差分流匹配与轨迹引导加权，强调密集的时间变化以及夹爪、被操作物体和接触区域移动的局部区域。在推理层面，DynaRoute预测词元级别的动态相关性并将其转化为注意力偏置，引导模型关注与控制相关的未来词元。在仿真和真实世界操作任务中的实验表明，DC-WAM持续提升策略性能，尤其是在光照、物体外观和背景纹理的分布外扰动下。
